...
export class RoleService {
  ...
  async create(
    ...
    this.logger.log('ROLE_CREATED_SUCCESS', {
      ...
      actorId: request.user.sub as string,
    });
    return new ResponseRoleDto(createdRole);
  }
  ...
  async update(
    ...
    return new ResponseRoleDto(updatedRole);
  }
  ...
  async resolveRoles(roleIds: string[]): Promise<Role[]> {
    const resolveQuery =
      roleIds.length === 0 ? { name: RoleEnum.Cashier } : { id: In(roleIds) };
